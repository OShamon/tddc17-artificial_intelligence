...
//choose a leaf node and remove it from the frontier
		  SearchNode firstNode = frontier.removeFirst();
...
//add the node to the explored set
		  ...
		  //get the child states (adjacent grid positions that are not walls) of firstNode
		  ArrayList<GridPos> childStates = p.getReachableStatesFrom(firstNode.getState());


		  for(GridPos childState: childStates){

//create a new search node from a state
		    SearchNode childNode = new SearchNode(childState, firstNode);

//only if not in the frontier or explored set
		    if(!(frontier.contains(childNode)) && !(explored.contains(childNode)) ){
//if the node contains a goal state then retur the corresponding solution
		      if (p.isGoalState(firstNode.getState()) == true)
		      {
			path = firstNode.getPathFromRoot();
			return path;
		      }
